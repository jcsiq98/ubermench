...
export class AdminService {
  ...
  async getStats() {
    ...
    const [
      ...
      this.prisma.income.count({ where: { createdAt: { gte: weekStart } } }),
      this.prisma.income.count({ where: { createdAt: { gte: monthStart } } }),
      this.prisma.expense.count(),
      this.prisma.appointment.count(),
      this.prisma.appointment.count({ where: { createdAt: { gte: todayStart } } }),
    ]);
    ...
    return {
      ...
      incomes: {
        total: totalIncomes,
        thisWeek: weekIncomes,
        thisMonth: monthIncomes,
      },
