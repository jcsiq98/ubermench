...
- [x] After provider marks service as complete, send customer:
  ```
...
- [x] After star selection, if 4-5: send interactive buttons for fine-grained selection (4 or 5)
- [x] After star selection, if 1-2: send interactive buttons for fine-grained selection (1 or 2)
- [x] After star selection, ask for comment:
  ```
  📝 Would you like to leave a comment? (Send "skip" to skip)
  ```
- [x] Save rating to `ratings` table
- [x] Update provider's `rating_average`:
  ```sql
...
- [x] After customer rates, send provider rating prompt:
  ```
  ⭐ How was working with Carlos?
  ```
  Same button flow as customer
- [x] Save rating (provider → customer)
- [x] Provider can skip rating with "skip" command

#### 6.3 — Review Display Integration
- [x] Ensure new reviews appear in provider detail cards (Milestone 3.3)
- [x] Reviews sorted by most recent
- [x] Show reviewer's first name only (privacy)

#### 6.4 — Request Lifecycle Completion
- [x] After rating, update `service_requests.status` → `completed`
- [x] Clear chat sessions from Redis
- [x] Clear booking session data
- [x] Return both parties to main menu / idle state
...
### Definition of Done
- ✅ Customer can rate provider (1-5 stars + optional comment) — **DONE**
- ✅ Provider can rate customer — **DONE**
- ✅ Ratings are persisted and averages recalculated — **DONE**
- ✅ New reviews appear in provider listings — **DONE**
- ✅ Request lifecycle fully completed — **DONE**
- ✅ Sessions cleaned up after completion — **DONE**
- ⬜ Code committed and pushed to GitHub
