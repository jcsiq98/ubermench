 ...
 * Ends chat session and transitions customer to the rating flow (M6).
 *
 * @param {string} requestId - service_requests.id
...
const markServiceComplete = async (requestId, providerPhone) => {
  try {
    ...
    // Get chat session to identify both parties
    const session = await getChatSession(providerPhone);
    if (session) {
      const { customerPhone, providerName, customerName } = session;

      // End chat session (removes Redis chat keys)
      await endChatSession(requestId, providerPhone);

      // ── M6: Resolve provider details for rating ──
      const request = await db('service_requests').where('id', requestId).first();
      let providerUserId = null;
      let providerId = null;
      if (request && request.provider_id) {
        providerId = request.provider_id;
        const providerRow = await db('providers').where('id', providerId).select('user_id').first();
        if (providerRow) providerUserId = providerRow.user_id;
      }

      // Resolve customer user id
      const customerUser = await db('users').where('phone', customerPhone).first();
      const customerId = customerUser ? customerUser.id : null;

      // ── M6: Send customer the rating prompt with buttons ──
      await whatsapp.sendTextMessage(
        customerPhone,
        `✅ *The service has been completed!*\n\n⭐ How would you rate *${providerName}*?\n\nTap a rating:`
      );

      await whatsapp.sendInteractiveButtons(customerPhone, 'Select your rating:', [
      ...
      await sessionManager.setSession(customerPhone, 'AWAITING_RATING', {
        requestId,
        providerId,
        providerUserId,
        providerName,
