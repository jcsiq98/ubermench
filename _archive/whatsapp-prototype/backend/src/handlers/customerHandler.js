...
const STATES = {
  ...
  // Milestone 6 — Ratings & Reviews
  AWAITING_RATING: 'AWAITING_RATING',
  AWAITING_RATING_FINE: 'AWAITING_RATING_FINE',
...
const handleCustomerMessage = async (phone, waName, message) => {
  ...
  // Allow providers to switch back to provider mode
  if (text === 'provider mode' || text === 'modo proveedor') {
    const existingUser = await findUserByPhone(phone);
    if (existingUser && existingUser.role === 'provider') {
      await sessionManager.clearSession(phone);
      const { handleProviderMessage } = require('./providerHandler');
      const dummyMessage = { type: 'text', text: { body: 'menu' } };
      return await handleProviderMessage(phone, waName, dummyMessage);
    }
  }
  ...
  switch (state) {
    ...
    // ── Milestone 6: Rating flow ──
    case STATES.AWAITING_RATING:
      return await handleRatingSelection(phone, waName, message, sessionData);

    case STATES.AWAITING_RATING_FINE:
      return await handleRatingFine(phone, waName, message, sessionData);

    case STATES.AWAITING_RATING_COMMENT:
      return await handleRatingComment(phone, waName, message, sessionData);
...
// ──────────────────────────────────────────────
// State handlers — Milestone 6: Ratings & Reviews
// ──────────────────────────────────────────────

/**
 * Handle the initial rating category selection.
 * Buttons: [⭐ 1-2 Poor] [⭐⭐⭐ 3 OK] [⭐⭐⭐⭐⭐ 4-5 Great]
 */
const handleRatingSelection = async (phone, waName, message, sessionData) => {
  if (message.type === 'interactive') {
    const content = extractInteractiveContent(message);
    if (content?.type === 'button_reply') {
      switch (content.id) {
        case 'rating_poor':
          // Ask for fine-grained: 1 or 2
          await whatsapp.sendInteractiveButtons(phone, '⭐ How many stars?', [
            { id: 'star_1', title: '⭐ 1 Star' },
            { id: 'star_2', title: '⭐⭐ 2 Stars' },
          ]);
          await sessionManager.setSession(phone, STATES.AWAITING_RATING_FINE, {
            ...sessionData,
            ratingCategory: 'poor',
          });
          return;
        ...
        case 'rating_ok':
          // 3 stars — go directly to comment
          await whatsapp.sendTextMessage(
            phone,
            `📝 Would you like to leave a comment? (Send *"skip"* to skip)`
          );
          await sessionManager.setSession(phone, STATES.AWAITING_RATING_COMMENT, {
            ...sessionData,
            stars: 3,
          });
          return;
        ...
        case 'rating_great':
          // Ask for fine-grained: 4 or 5
          await whatsapp.sendInteractiveButtons(phone, '⭐ How many stars?', [
            { id: 'star_4', title: '⭐⭐⭐⭐ 4 Stars' },
            { id: 'star_5', title: '⭐⭐⭐⭐⭐ 5 Stars' },
          ]);
          await sessionManager.setSession(phone, STATES.AWAITING_RATING_FINE, {
            ...sessionData,
  ...
  // Unrecognized input — resend the buttons
  await whatsapp.sendTextMessage(
    phone,
    `🤔 Please tap one of the rating buttons below, or type "cancel" to skip rating.`
  );
  await whatsapp.sendInteractiveButtons(phone, 'Select your rating:', [
    { id: 'rating_poor', title: '⭐ 1-2 Poor' },
    { id: 'rating_ok', title: '⭐⭐⭐ 3 OK' },
    { id: 'rating_great', title: '⭐⭐⭐⭐⭐ 4-5 Great' },
  ]);
};

/**
 * Handle fine-grained star selection (1 vs 2, or 4 vs 5).
 */
const handleRatingFine = async (phone, waName, message, sessionData) => {
  if (message.type === 'interactive') {
    const content = extractInteractiveContent(message);
    if (content?.type === 'button_reply') {
      const starMap = {
  ...
  // Unrecognized input — resend the appropriate buttons
...
/**
 * Handle comment input after star selection.
 * User can type a comment or send "skip".
 */
const handleRatingComment = async (phone, waName, message, sessionData) => {
  if (message.type !== 'text') {
  ...
  const providerName = sessionData.providerName;

  try {
    // Insert rating (customer → provider)
    ...
    console.log(`[CustomerHandler] Rating saved: ${stars} stars for provider ${providerUserId} by customer ${customerId}`);

    // ── Update provider's rating_average ──
    await updateProviderRatingAverage(providerUserId);

    // ── Send confirmation ──
    const starString = '⭐'.repeat(stars);
    let confirmMsg = `✅ *Thank you for your review!*\n\n`;
    confirmMsg += `${starString} (${stars}/5)\n`;
    if (comment) {
      confirmMsg += `💬 "${comment}"\n\n`;
    } else {
      confirmMsg += `\n`;
    }
    confirmMsg += `Your feedback helps other customers find great providers.`;

    await whatsapp.sendTextMessage(phone, confirmMsg);

    // ── Trigger provider rating (M6.2) ──
    await triggerProviderRating(sessionData);

    // ── Return customer to menu ──
    await whatsapp.sendInteractiveButtons(phone, 'What would you like to do?', [
      { id: 'btn_new_service', title: '🏠 Back to Menu' },
      { id: 'btn_my_requests', title: '📄 My Requests' },
    ]);

    // Clear to a resting state
    await sessionManager.setSession(phone, STATES.WELCOME, {
      userId: customerId,
      name: sessionData.customerName || waName,
      isReturning: true,
    });

  } catch (error) {
    console.error('[CustomerHandler] Error saving rating:', error.message);
    await whatsapp.sendTextMessage(phone, `❌ Something went wrong saving your review. Please try again.`);
  }
};

/**
 * Recalculate and update the provider's rating_average in both users and providers tables.
 * @param {string} providerUserId - the user_id of the provider
 */
const updateProviderRatingAverage = async (providerUserId) => {
  try {
    const result = await db('ratings')
      .where('ratee_id', providerUserId)
    ...
    console.log(`[CustomerHandler] Provider ${providerUserId} rating updated: ${avgRating} (${totalRatings} reviews)`);
  } catch (error) {
    console.error('[CustomerHandler] Error updating provider rating average:', error.message);
  }
};

/**
 * After the customer finishes rating, trigger the provider's rating flow.
 * Sends a rating prompt to the provider so they can rate the customer.
 * @param {object} sessionData - must include requestId, providerId, providerUserId, customerId, customerName
 */
const triggerProviderRating = async (sessionData) => {
  try {
    const { requestId, providerId, customerId, customerName } = sessionData;

    if (!providerId || !customerId) {
      console.warn('[CustomerHandler] Missing data to trigger provider rating');
      return;
    }
    ...
    const provider = await db('providers')
      ...
      .select('users.phone', 'users.name', 'providers.user_id')
      .first();

    if (!provider) {
      console.warn('[CustomerHandler] Provider not found for rating trigger');
      return;
    }

    // Send rating prompt to provider
    await whatsapp.sendTextMessage(
      provider.phone,
      `⭐ *How was working with ${customerName || 'the customer'}?*\n\nTap a rating:`
    );

    await whatsapp.sendInteractiveButtons(provider.phone, 'Rate the customer:', [
      { id: 'prating_poor', title: '⭐ 1-2 Poor' },
      { id: 'prating_ok', title: '⭐⭐⭐ 3 OK' },
      { id: 'prating_great', title: '⭐⭐⭐⭐⭐ 4-5 Great' },
    ]);

    // Set provider session to AWAITING_PROVIDER_RATING
    // (handled by providerHandler)
    const sessionManager = require('../services/sessionManager');
    await sessionManager.setSession(provider.phone, 'AWAITING_PROVIDER_RATING', {
      requestId,
      providerId,
    ...
    console.log(`[CustomerHandler] Provider rating prompt sent to ${provider.phone}`);
  } catch (error) {
    console.error('[CustomerHandler] Error triggering provider rating:', error.message);
  }
};
...
const handleMenu = async (phone, waName) => {
  ...
  if (existingUser) {
    const isAlsoProvider = existingUser.role === 'provider';
    const switchHint = isAlsoProvider ? `\n\n💡 Type *"provider mode"* to switch back to your provider dashboard.` : '';
    await whatsapp.sendTextMessage(phone, `📋 Main Menu — What service do you need, ${existingUser.name}?${switchHint}`);
  } else {
...
const handleHelp = async (phone) => {
  // Check if user is also a provider to show switch option
  const existingUser = await findUserByPhone(phone);
  const isAlsoProvider = existingUser && existingUser.role === 'provider';
  const switchHint = isAlsoProvider ? `\n👤 *"provider mode"* — Switch to your provider dashboard\n` : '';
